...
import { Mail, Lock, Eye, EyeOff, ArrowRight, Sparkles, AlertCircle, ShieldCheck, ArrowLeft } from 'lucide-react';
...
export default function Login() {
  ...
  const handleVerify2FA = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userRole = localStorage.getItem('justme_role') || 'user';
      notify('success', 'Verificación exitosa', 'Sesión iniciada correctamente.');

      if (userRole === 'admin') navigate('/admin');
      else if (userRole === 'professional') navigate('/professional');
      else navigate('/user');
    } catch (err: any) {
      setApiError('Código de verificación inválido.');
    } finally {
      setLoading(false);
    }
  };
            ...
            {!show2FA ? (
              ...
              >
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                  <div style={{ 
                    width: 64, height: 64, borderRadius: '50%', background: 'var(--primary-50)', color: 'var(--primary-600)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem auto'
                  }}>
                    <ShieldCheck size={32} />
                  </div>
                  <h1>Verificación 2FA</h1>
                  <p className="login-subtitle">Introduce el código de 6 dígitos de tu aplicación de autenticación.</p>
                </div>
                ...
                <form onSubmit={handleVerify2FA} className="login-form">
                  ...
                  <Button type="submit" fullWidth loading={loading} size="lg" disabled={twoFactorCode.length !== 6}>
                    Verificar y Entrar
                  </Button>
                  ...
                  <button 
                    ...
                    onClick={() => { setShow2FA(false); setApiError(null); }}
                    style={{ 
                      width: '100%', background: 'none', border: 'none', color: 'var(--neutral-500)', 
                      marginTop: '1.5rem', cursor: 'pointer', display: 'flex', alignItems: 'center', 
